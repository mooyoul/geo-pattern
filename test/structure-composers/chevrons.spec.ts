...
import { ChevronsComposer } from "../../src/structure-composers/chevrons";

describe(ChevronsComposer.name, () => {
  let pattern: Pattern;
  let preset: PatternPreset;
  let composer: ChevronsComposer;

  beforeEach(() => {
    pattern = new Pattern();
    ({ composer, preset } = createComposer(ChevronsComposer));
  });
  ...
  describe("#compose", () => {
    it("should compose pattern", async () => {
      ...
      expect(structure.name).to.eq("chevrons");
      ...
      const expected = await readFixture("structures/chevrons.svg");
